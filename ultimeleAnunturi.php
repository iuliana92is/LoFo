<article class="ultimeleAnunturi">
    <h1>Ultimele anunturi adaugate</h1>
    <section id="ultimeleAnunturi">
        <div class="ult">
            <ul>
                <li id="categorie-ultimele">Categorie</li>
                <li id="zona-ultimele">zona</li>
                <li id="perioada-ultimele">perioada</li>
                <li id="recompensa-ultimele">obiect/animal</li>
                <li id="detalii-ultimele"> detalii</li>
            </ul>
        </div>
        <div class="listaObiecte">
            <ul>
                <li class="categorie" >obiect </li>
                <li class="zona"> dancu </li>
                <li class="dataPublicarii" > ziua curenta </li>
                <li class="obiect/animal" > telefon </li>
                <li class="detalii">  <img src="assets/images/icons/down.png" alt="down arrow"  onclick="addClass('.infoText ', 'visible')" />   Detalii </li>
            </ul>
            <!-- details informations -->
            <div class="clearfix"></div>
            <div class="infoText hidden">
                <div class="infoTextObiect">
                    <div class="leftText">
                        <p>culoare: </p>
                    </div>
                    <div class="rightText">
                        <p> maro</p>
                    </div>
                    <div class="clearfix"></div>
                    <div class="leftText">
                        <p>stare: </p>
                    </div>
                    <div class="rightText">
                        <p> stricat</p>
                    </div>
                    <div class="clearfix"></div>
                    <div class="leftText">
                        <p>detalii: </p>
                    </div>
                    <div class="rightText">
                        <p>   verde buline  buline bulinealbastre   verde buline albastre verde buline albastreverde buline albastre  verde buline albastre verde buline albastre  verde buline albastre verde buline albastre  verde buline albastre verde buline albastre </p>
                    </div>
                    <div class="clearfix"></div>
                </div>
                <div class="infoTextImg">
                    <img src="assets/images/lost/lost2.jpg" alt="imagine obiect" />
                </div>
                <div class="clearfix"></div>
                <div class="infoTextOpen">
                    <img src="assets/images/icons/up.png" alt="up arrow" onclick="removeClass('.infoText ', 'visible')"/> 
                </div>
            </div>
            <div class="clearfix"></div>
            <hr/>
            <!-- close detail informations -->
            <ul>
                <li class="categorie" >obiect </li>
                <li class="zona"> dancu </li>
                <li class="dataPublicarii" > ziua curenta </li>
                <li class="obiect/animal" > buletin </li>
                <li class="detalii">  <img src="assets/images/icons/down.png" alt="down arrow"  onclick="addClass('.infoText ', 'visible')" />   Detalii </li>
            </ul>
            <!-- details informations -->
            <div class="clearfix"></div>
            <div class="infoText hidden">
                <div class="infoTextObiect">
                    <div class="leftText">
                        <p>culoare: </p>
                    </div>
                    <div class="rightText">
                        <p> maro</p>
                    </div>
                    <div class="clearfix"></div>
                    <div class="leftText">
                        <p>stare: </p>
                    </div>
                    <div class="rightText">
                        <p> stricat</p>
                    </div>
                    <div class="clearfix"></div>
                    <div class="leftText">
                        <p>detalii: </p>
                    </div>
                    <div class="rightText">
                        <p>   verde buline  buline bulinealbastre   verde buline albastre verde buline albastreverde buline albastre  verde buline albastre verde buline albastre  verde buline albastre verde buline albastre  verde buline albastre verde buline albastre </p>
                    </div>
                    <div class="clearfix"></div>
                </div>
                <div class="infoTextImg">
                    <img src="assets/images/lost/lost4.jpg" alt="imagine obiect" />
                </div>
                <div class="clearfix"></div>
                <div class="infoTextOpen">
                    <img src="assets/images/icons/up.png" alt="up arrow" onclick="removeClass('.infoText ', 'visible')"/> 
                </div>
            </div>
            <div class="clearfix"></div>
            <hr/>
            <!-- close detail informations -->
            <ul>
                <li class="categorie" >obiect </li>
                <li class="zona"> dancu </li>
                <li class="dataPublicarii" > ziua curenta </li>
                <li class="obiect/animal" > papagal </li>
                <li class="detalii">  <img src="assets/images/icons/down.png" alt="down arrow"  onclick="addClass('.infoText ', 'visible')" />   Detalii </li>
            </ul>
            <!-- details informations -->
            <div class="clearfix"></div>
            <div class="infoText hidden">
                <div class="infoTextObiect">
                    <div class="leftText">
                        <p>culoare: </p>
                    </div>
                    <div class="rightText">
                        <p> maro</p>
                    </div>
                    <div class="clearfix"></div>
                    <div class="leftText">
                        <p>stare: </p>
                    </div>
                    <div class="rightText">
                        <p> stricat</p>
                    </div>
                    <div class="clearfix"></div>
                    <div class="leftText">
                        <p>detalii: </p>
                    </div>
                    <div class="rightText">
                        <p>   verde buline  buline bulinealbastre   verde buline albastre verde buline albastreverde buline albastre  verde buline albastre verde buline albastre  verde buline albastre verde buline albastre  verde buline albastre verde buline albastre </p>
                    </div>
                    <div class="clearfix"></div>
                </div>
                <div class="infoTextImg">
                    <img src="assets/images/lost/lost3.jpg" alt="imagine obiect" />
                </div>
                <div class="clearfix"></div>
                <div class="infoTextOpen">
                    <img src="assets/images/icons/up.png" alt="up arrow" onclick="removeClass('.infoText ', 'visible')"/> 
                </div>
            </div>
            <div class="clearfix"></div>
            <hr/>
            <!-- close detail informations -->
        </div>
    </section>
</article>